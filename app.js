// OddFynds - Modern Thrifted Fashion Store
...
class OddFyndsStore {
    ...
    renderFeaturedProducts() {
        ...
        container.innerHTML = featuredProducts.map(product => `
            <div class="group cursor-pointer" onclick="this.viewProduct(${product.id})">
                <div class="relative overflow-hidden rounded-2xl mb-4">
                    ...
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        <button onclick="event.stopPropagation(); addToCart(${product.id})" 
                                class="bg-white text-gray-900 px-6 py-3 rounded-full font-semibold hover:bg-gray-100 transition-colors flex items-center space-x-2">
    ...
    setupScrollAnimations() {
        ...
        document.querySelectorAll('.group, .space-y-4, .grid').forEach(el => {
            observer.observe(el);
        });
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    window.oddFyndsStore = new OddFyndsStore();
    console.log('🛍️ OddFynds Store Loaded Successfully! ✨');
});
